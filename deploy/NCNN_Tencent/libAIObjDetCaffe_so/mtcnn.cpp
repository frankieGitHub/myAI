...
//		model_path+"/det3.param",
        model_path+"/mobilenetSSD_120000.param"
//        model_path+"/mobilenetSSD_baidu15000.param"
	};
...
//		model_path+"/det2.bin",
//		model_path+"/det3.bin",
        model_path+"/mobilenetSSD_120000.bin"
//        model_path+"/mobilenetSSD_baidu15000.bin"
	};
...
//	Onet.load_model(bin_files[2].data());
    MobileNetSSDnet.load_param(param_files[0].data());
    MobileNetSSDnet.load_model(bin_files[0].data());
}

MTCNN::MTCNN(const unsigned char* bin_mem, const unsigned char* param_mem) {
    MobileNetSSDnet.load_param(param_mem);
    MobileNetSSDnet.load_model(bin_mem);
}
...
/*
string objLabels[] = {"background",
                               "aeroplane", "bicycle", "bird", "boat",
                               "bottle", "bus", "car", "cat", "chair",
                               "cow", "diningtable", "dog", "horse",
                               "motorbike", "person", "pottedplant",
                               "sheep", "sofa", "train", "tvmonitor",
                               "flowers", "sunrise", "beach", "pizza",
                               "bread", "bowl", "soup", "grass", "plate", "sky"
};*/

void MTCNN::SSDDetect(ncnn::Mat& img_, std::vector<float>& objInfo_, int* len) {
    ...
    ncnn::Mat in;
//    LOGE("SSD detect enter");
//    printf("SSD detect enter\n");
    resize_bilinear(img, in, 300, 300);
    ncnn::Extractor ex = MobileNetSSDnet.create_extractor();
    ex.set_num_threads(num_threads);
    ex.set_light_mode(true);
//    ex.input("data", in);
    ex.input(0, in);    ///0-"data"
    ncnn::Mat out;
//    ex.extract("detection_out", out);
    ex.extract(149, out); //149-"detection_out"

//    LOGE("out channel is: %d", out.c);
//    LOGE("out h is: %d", out.h);
//    LOGE("out w is: %d", out.w);

    int count = 0;
    *len = out.h;

//    for(int i=1; i<=out.c; i++) {
        for(int j=0; j<out.h; j++) {
            for(int k=0; k<out.w; k++) {
                float* values = out.row(j);
                if(k == 2 || k == 4) objInfo_.push_back((*(values + k)) * img_w);
                else if(k == 3 || k == 5) objInfo_.push_back((*(values + k)) * img_h);
                else objInfo_.push_back(*(values + k));

//                LOGE("setting value: %f", values[k]);
            }
        }
//    }
...
//        LOGE("label is: %d", (int)values[0]);
//        LOGE("prob is: %f", values[1]);
//        LOGE("x1: %f", values[2]*img_w);
//        LOGE("y1: %f", values[3]*img_h);
//        LOGE("x2: %f", values[4]*img_w);
//        LOGE("y2: %f", values[5]*img_h);
