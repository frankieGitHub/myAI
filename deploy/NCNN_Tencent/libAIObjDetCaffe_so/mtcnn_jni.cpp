...
JNIEXPORT jboolean JNICALL
Java_com_ai_camera_caffemodel_CaffeModel_DetectionModelInit(JNIEnv *env, jobject instance,
                                                                jstring faceDetectionModelPath_) {
    return Java_com_mtcnn_1as_MTCNN_FaceDetectionModelInit(env, instance, faceDetectionModelPath_);
}


JNIEXPORT jboolean JNICALL
Java_com_ai_camera_caffemodel_CaffeModel_SSDInit(JNIEnv *env, jobject instance, jbyteArray param_mem,
                                              jbyteArray bin_mem) {

    LOGE("caffe model SSD init");

    // init param
    {
        int tParamMemLen = env->GetArrayLength(param_mem);
        LOGE("caffe model param  length=%d",tParamMemLen);
        ncnn_param.create(tParamMemLen, (size_t) 1u);
        env->GetByteArrayRegion(param_mem, 0, tParamMemLen, (jbyte *) ncnn_param);
    }

    // init bin
    {
        int tBinMemLen = env->GetArrayLength(bin_mem);
        LOGE("caffe model bin  length=%d",tBinMemLen);
        ncnn_bin.create(tBinMemLen, (size_t) 1u);
        env->GetByteArrayRegion(bin_mem, 0, tBinMemLen, (jbyte *) ncnn_bin);
    }

    mtcnn = new MTCNN((const unsigned char *)ncnn_bin, (const unsigned char *)ncnn_param);

//    env->ReleaseByteArrayElements(bin_mem, (jbyte *)ncnn_bin, 0);
//    env->ReleaseByteArrayElements(param_mem, (jbyte *)ncnn_param, 0);
    detection_sdk_init_ok = true;
...
Java_com_mtcnn_1as_MTCNN_SSDDetect(JNIEnv *env, jobject instance, jbyteArray imageDate_,
                                    jint imageWidth, jint imageHeight, jint imageChannel, jintArray numObject) {

    //  LOGD("JNI开始检测人脸");
//    LOGE("SSD detect jni enter");
    if(!detection_sdk_init_ok){
        LOGD("ssd detect not init yet, return");
        return NULL;
    }
...
//        ncnn_img = ncnn::Mat::from_pixels(faceImageCharDate, ncnn::Mat::PIXEL_RGBA2RGB, imageWidth, imageHeight);
        ncnn_img = ncnn::Mat::from_pixels(faceImageCharDate, ncnn::Mat::PIXEL_RGBA2BGR,
                                          imageWidth, imageHeight);
    }


//    jint *numObj_ = env->GetIntArrayElements(numObject, NULL);
//    if (NULL == numObj_){
//        LOGD("导入数据为空，直接返回空");
//        env->ReleaseIntArrayElements(numObject, numObj_, 0);
//        return NULL;
//    }

    int* getNumObj = new int[1];

    std::vector<float> objInfo;

    mtcnn->SSDDetect(ncnn_img, objInfo, getNumObj);

    env->SetIntArrayRegion(numObject, 0, 1, getNumObj);

    float* objInfoTemp =  new float[6*getNumObj[0]];
//    LOGE("SSD obj num is: %d", getNumObj[0]);
    for(int i=0; i<6*getNumObj[0]; i++) {
        objInfoTemp[i] = objInfo[i];
//        LOGE("data[%d] is: %f", i, objInfoTemp[i]);
    }

    jfloatArray tObjInfo = env->NewFloatArray(6*getNumObj[0]);
    env->SetFloatArrayRegion(tObjInfo,0,6*getNumObj[0],objInfoTemp);
//    if(objInfo != NULL) free(objInfo);
    free(objInfoTemp);
    free(getNumObj);
    env->ReleaseByteArrayElements(imageDate_, imageDate, 0);
...
JNIEXPORT jfloatArray JNICALL
Java_com_ai_camera_caffemodel_CaffeModel_SSDDetect(JNIEnv *env, jobject instance, jbyteArray imageDate_,
                                                   jint imageWidth, jint imageHeight, jint imageChannel, jintArray numObject) {
    return Java_com_mtcnn_1as_MTCNN_SSDDetect(env, instance, imageDate_, imageWidth, imageHeight, imageChannel, numObject);
}
